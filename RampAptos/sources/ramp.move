module RampAptos::ramp {
    ...
    public entry fun add_coin<CoinType>(owner: &signer, fee: u64, amount: u64) acquires GlobalStorage, RampEventStore {
        ...
        simple_map::add(
            &mut global_storage.coin_vaults,
            get_coin_address<CoinType>(),
            CoinVault {
                store_amount: amount,
                coin_revenue: 0u64,
        ...
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_addr).coin_added_event_handle,
            CoinAddedEvent {
    ...
    public entry fun remove_coin<CoinType>(owner: &signer, recipient: address) acquires GlobalStorage, RampEventStore {
        ...
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_addr).coin_removed_event_handle,
            CoinRemovedEvent {
                coin_address: get_coin_address<CoinType>(),
                amount: coin_balance,
                recipient,
    ...
    public entry fun offramp_coin<CoinType>(
        ...
        let coin_vault = simple_map::borrow_mut(&mut borrow_global_mut<GlobalStorage>(obj_addr).coin_vaults, &get_coin_address<CoinType>());
        let fee = coin_vault.coin_fee_percentage;
        let fee_amount = amount * fee / 100;

        coin_vault.coin_revenue += fee_amount;
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_addr).coin_offramp_event_handle,
            CoinOfframpEvent {
                coin_address: get_coin_address<CoinType>(),
                coin_name: coin::symbol<CoinType>(),
    ...
    public entry fun onramp_coin<CoinType>(
        ...
        let balance = coin::balance<CoinType>(obj_addr);
        let coin_vault = simple_map::borrow(&global_storage.coin_vaults, &get_coin_address<CoinType>());

        assert!(balance - coin_vault.coin_revenue >= amount, error::invalid_argument(EINSUFICIENT_ASSET_BALANCE));
    ...
    public entry fun add_asset(owner: &signer, asset: Object<Metadata>, fee: u64, initial_amount: u64) acquires GlobalStorage, RampEventStore {
        ...
        let owner_store = ensure_primary_store_exists(signer::address_of(owner), asset);
        ...
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_addr).asset_added_event_handle,
            AssetAddedEvent {
                asset_address: asset,
                fee_percentage: fee,
    ...
    public entry fun remove_asset(owner: &signer, asset: Object<Metadata>, asset_recipient: address) acquires GlobalStorage, RampEventStore {
        ...
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_addr).asset_removed_event_handle,
            AssetRemovedEvent {
                asset_address: asset,
                balance: balance,
    ...
    public entry fun set_contract_state(owner: &signer, state: bool) acquires GlobalStorage, RampEventStore {
        ...
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_addr).contract_state_changed_event_handle,
            ContractStateChangedEvent { new_state: state }
        );
    }
    ...
    public entry fun set_owner(owner: &signer, new_owner: address) acquires GlobalStorage, RampEventStore {
        ...
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_address).owner_changed_event_handle,
    ...
    public entry fun off_ramp_deposit(
        ...
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_address).deposit_event_handle,
            RampDeposit {
                asset: asset,
                asset_name: fungible_asset::symbol(asset),
    ...
    public entry fun on_ramp_withdraw(admin: &signer, asset: Object<Metadata>, recipient: address, amount: u64) acquires GlobalStorage, RampEventStore {
        ...
        assert!(
            fungible_asset::balance(asset_vault.store) > amount - asset_vault.asset_revenue,
        ...
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_address).withdraw_event_handle,
            RampWithdraw {
                asset: asset,
                amount: amount,
                recipient: recipient
    ...
    public entry fun set_fee(admin: &signer, asset: Object<Metadata>, new_fee: u64) acquires GlobalStorage, RampEventStore {
        ...
        event::emit_event(
            &mut borrow_global_mut<RampEventStore>(obj_address).asset_fee_changed_event_handle,
            AssetFeeChanged {
                asset: asset,
                new_fee: new_fee
            }
    ...
    fun test_on_ramp_deposit(owner: signer, admin: address, user_one: address) acquires GlobalStorage, RampEventStore {
        ...
        assert!(event::was_event_emitted_by_handle(&borrow_global<RampEventStore>(get_obj_address()).deposit_event_handle, &RampDeposit {
            ...
            asset_name: fungible_asset::symbol(test_info.metadata),
